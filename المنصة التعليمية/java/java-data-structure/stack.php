...
<!-- Mirrored from harmash.com/java/java-data-structure/stack.php by HTTrack Website Copier/3.x [XR&CO'2014], Sat, 13 Jun 2020 17:01:16 GMT -->
...
<head>
    ...
    <title>الكلاس Stack في جافا</title>
...
<!--<p class="center red b" style="padding-top: 20px; font-size: 14px;">لا زلنا نعمل على إعداد دروس جديدة</p>-->
            ...
            <div id="main">
                ...
                <h1><span class="session">Java</span>الكلاس <code data-enlighter-language="java">Stack</code> في جافا</h1>
<article>
    <h2>مقدمة</h2>
	<p>الكلاس <code data-enlighter-language="java">Stack</code> يرث من الكلاس <code data-enlighter-language="java">Vector</code>, و هو يطبق المبدأ <span class="word">LIFO</span> <span class="inline">( <span class="word">Last In First Out</span> )</span>, أي العنصر الذي يدخل في الأخير يخرج في الأول.</p>
    <p>بما أنه يرث من الكلاس <code data-enlighter-language="java">Vector</code>, فهذا يعني أنه يملك جميع الدوال الموجودة فيه.</p>
	<p>هنا يتم تخزين العناصر بشكل عامودي, حيث أن العناصر تترتب فيه من الأسفل إلى الآعلى.<br>
	إذاً عند إضافة عنصر ( كائن ), سيتم تخزينه في أعلى الـ <code data-enlighter-language="java">Stack</code>.  و عند حذف عنصر, سيتم حذف العنصر الموجود في الأعلى أيضاً.<br>
	بما أنه لا يمكن التعامل معه إلا من الأعلى فهذا يعني أنه يمكن الوصول فقط للعنصر الموجود في أعلى الـ <code data-enlighter-language="java">Stack</code>.</p>
	<br>
	<p>ننصحك بمشاهدة هذا الفيديو حتى تفهم طريقة تخزين الكائنات و استرجاعها في الـ <code data-enlighter-language="java">Stack</code>.</p>
	<video controls class="border">
		<source src="how-data-stored-in-the-stack.mp4" type="video/mp4">
		متصفحك لا يدعم صيغة الفيديو!
	</video>
</article>
<article>
    <h2>دوال الكلاس <code data-enlighter-language="java">Stack</code></h2>
	<p>الجدول التالي يحتوي على دوال الكلاس <span class="word">Stack</span>.</p>
	<table class="list full-width rows-numbers">
	    <tr>
			<th colspan="2">الدالة مع تعريفها</th>
		</tr>
		<tr>
		    <td><span class="code-title"><code data-enlighter-language="java">public boolean empty()</code></span>
			تفحص كائن الـ <code data-enlighter-language="java">Stack</code> لمعرفة إذا كان يحتوي على عناصر أم لا.<br>
			ترجع <code data-enlighter-language="java">true</code> في حال كان كائن الـ <code data-enlighter-language="java">Stack</code> لا يزال يحتوي على عناصر.</td>
		</tr>
		<tr>
		    <td><span class="code-title"><code data-enlighter-language="java">public Object peek()</code></span>
			ترجع العنصر الموجود في أعلى كائن الـ <code data-enlighter-language="java">Stack</code>, بدون أن تحذفه.</td>
		</tr>
		<tr>
		    <td><span class="code-title"><code data-enlighter-language="java">public Object pop()</code></span>
			ترجع العنصر الموجود في أعلى كائن الـ <code data-enlighter-language="java">Stack</code>, و تحذفه منه.</td>
		</tr>
		<tr>
		    <td><span class="code-title"><code data-enlighter-language="java">public Object push(Object element)</code></span>
			تضع الكائن الذي نمرره لها كـ <span class="word">Argument</span> في أعلى كائن الـ <code data-enlighter-language="java">Stack</code>, و ترجعه أيضاً.</td>
		</tr>
		<tr>
		    <td><span class="code-title"><code data-enlighter-language="java">public int search(Object element)</code></span>
			تبحث عن الكائن الذي نمرره لها كـ <span class="word">Argument</span> في جميع عناصر كائن الـ <code data-enlighter-language="java">Stack</code>.<br>
			في حال تم إيجاد تطابق, ترجع رقمه التراتبي بالنسبة للعناصر الموجودة في كائن الـ <code data-enlighter-language="java">Stack</code>.<br>
			في حال عدم إيجاد تطابق, ترجع <span class="number">1-</span></td>
		</tr>
	</table>
</article>
<article>
    <h2>مثال شامل</h2>
	    <p>في المثال التالي قمنا بتعريف كائن نوعه <code data-enlighter-language="java">Stack</code> ثم أدخلنا فيه أربعة عناصر <code data-enlighter-language="java">A</code> - <code data-enlighter-language="java">B</code> - <code data-enlighter-language="java">C</code> - <code data-enlighter-language="java">D</code>.</p>
		<p>بعدها قمنا بالبحث عنهم بداخل كائن الـ <code data-enlighter-language="java">Stack</code> حتى نعرف مكان وجودهم بداخله, أي كيف تم ترتيبهم.<br>
		قمنا أيضاً بالبحث عن كائنات غير موجودة في الـ <code data-enlighter-language="java">Stack</code> حتى نرى النتيجة التي ترجع لنا عند البحث عن قية غير موجودة فيه.</p>
		<p>في الأخير قمنا بعرض و حذف جميع العناصر الموجودة فيه.</p>
		<br>
...
<!-- Mirrored from harmash.com/java/java-data-structure/stack.php by HTTrack Website Copier/3.x [XR&CO'2014], Sat, 13 Jun 2020 17:01:17 GMT -->
</html>